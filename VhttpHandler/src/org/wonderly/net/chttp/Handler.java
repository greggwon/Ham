...
/**
 *  This is the protocol handler for the chttp protocol.
 *  This protocol caches http:// requests in a local directory, and has a
 *  listener mechanism to allow downloads to be visualised.
 ...
 *  <pre>
		...
		org.wonderly.url.chttp.Handler.addDownloadListener( new DefaultDownloadListener( top ) );
		// Set caching directory as 'cache' under the running directory.
		org.wonderly.url.chttp.Handler.setCacheDir("cache");
 *  </pre>
 *  <p>
 *
 *  The following properties are consulted in a static initializer
 *  <dl>
 *		<dt>org.wonderly.chttp.protocol<dd>defaults to http
 *		<dt>org.wonderly.chttp.cache<dd>defaults to no cache directory, which is null
 *		<dt>org.wonderly.chttp.debug<dd>set to non-empty string enables debug logging
 *		    using the <code>org.wonderly.url.chttp.Handler</code> logger
 *  </dl>
 *
...
//	static JFrame parent;
	...
	public static void setCacheRoot(String cacheRoot) {
		Handler.cacheRoot = cacheRoot;
	}
	private static volatile int connTimeout = -1;
	...
	static {
		String pr = System.getProperty("org.wonderly.chttp.protocol");
		if( pr != null )
			proto = pr;
		String cd = System.getProperty("org.wonderly.chttp.cache");
		if( cd != null )
			cache = cd;
		Logger log = Logger.getLogger(Handler.class.getName());
		if( log.isLoggable(Level.FINE)) {
			log.fine("protocol from \"org.wonderly.chttp.protocol\" system property: "+proto );
			log.fine("cache from \"org.wonderly.chttp.cache\" system property: "+cache );
		}
	}
	...
	public static void main( String args[] ) throws Exception {
		...
		//Logger.getLogger("").getHandlers()[0].setFormatter( new StreamFormatter() );
		...
		top.setVisible(true);
		org.wonderly.net.chttp.Handler.addDownloadListener( new DefaultDownloadListener( top ) );
		org.wonderly.net.chttp.Handler.setCacheDir("c:/test-cache");
...
//				new File( "cache/reggie-dl.jar" ).delete();
		...
		for( int i = 0; i < 100; ++i ) {
			log.info("connecting: ["+i+"]");
			URL u = new URL( "chttp://chipmunk:8090/reggie-dl.jar" );
			try {
				...
				log.log(Level.SEVERE, "could not open: "+ex.toString(), ex);
				JOptionPane.showMessageDialog( top, ex );
				break;
			}
		}
	}
	...
	public static void setConnectTimeout( int val ) {
		connTimeout = val;
	}
	...
	private class ProxyUrlConnection extends URLConnection {
		...
		private synchronized URLConnection getReal() throws IOException {
			if( realcon == null ) {
				realcon = openRealConnection(svurl);
	...
	private URLConnection openRealConnection( final URL url ) throws IOException {
		log.log(Level.FINEST,"openConnection: "+url, new Throwable(url.toString()) );
		final IOException exc[] = new IOException[1];
		Object lk = ulocks.get( url );
		if( lk == null ) {
			lk = new Object();
			ulocks.putIfAbsent( url, lk);
		}
		URLConnection ret;
		synchronized( ulocks.get( url ) ) {
			ret = (URLConnection) AccessController.doPrivileged(
		...
		if( ret == null && exc[0] != null ) {
			log.info("throwing exception "+exc[0]);
			throw exc[0];
		}
		return ret;
	}
	...
	private static ConcurrentHashMap<URL,Object> ulocks = new ConcurrentHashMap<URL,Object>();
	...
	private java.net.URLConnection openIt( URL url ) throws IOException {
		...
		// Don't check on files time stamp and size on every connect for 'dltimeout' intervals between
		// dltimeout is 30 seconds or less usually so that short bursts of multiple accesses will
		// not cause multiple downloads.
		if( lastTime == null || (System.currentTimeMillis() - lastTime.longValue()) > dltimeout ) {
			...
			try {
				...
				try {
					if( failFast ) {
						throw new IOException( url.toString()+": connection attempt aborted by application" );
					}
					tmpIs = uc.getInputStream();
				} catch( IOException ex ) {
					lastex = (IOException)new ConnectException( ex.getMessage()+": "+u).initCause( ex );
					if( log.isLoggable( Level.FINER ) )
						log.log( Level.FINER, ex.toString()+": "+u, ex );
					else if( log.isLoggable( Level.FINE ) )
						log.fine( ex.toString()+": "+u );
				} finally {
					try {
						if (tmpIs != null)
							tmpIs.close();
					} catch (Exception exx) {
						log.log(Level.FINER, exx.toString(), exx);
					}
				}
				if( log.isLoggable(Level.FINER))
					log.finer("Tried opening connection to "+u +", tmpIs="+tmpIs );
		
				if( file.exists() && tmpIs != null ) {
					if( log.isLoggable(Level.FINE)) log.fine( (tmpIs == null ? "Failed" : "Got")+
						" connection, checking cached file ("+file+") info for "+u );
					...
					// Make sure we connected and have valid sizes and times
//					if( tmpIs != null ) {
						// If time or size are different, delete the cached file.
						if( ulast > flast ) {
							file.delete();
...
//					}
				} else if( lastex != null && file.exists() == false ) {
					log.info("no cached file and could not open connection to: "+u+", rethrowing: "+lastex );
					throw lastex;
				}
				if( tmpIs == null ) {
					if( file.exists() ) {
						log.fine("could not connect, using cached file: "+file+" for "+u);
					}
				}
				if( file.exists() ) {
					// file is up to date, mark time of last check.
					lastget.put( url.toString(), new Long( System.currentTimeMillis() ) );
//				} else {
//					if (log.isLoggable(Level.FINE)) {
//						log.fine("Did not find cached file!: "+file);
//					}
				}
			} finally {
		...
		if( file.exists() == false ) {
			if( log.isLoggable(Level.FINE)) log.fine("No local file, downloading: "+u);
